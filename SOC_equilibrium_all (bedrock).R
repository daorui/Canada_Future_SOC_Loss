...
# ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20241230.tif') # this version is for bd 25% percent peat and soc for 4 category peat
# ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20250107.tif') #20250120 181.1869 total + 157.1045 PG landcover 
ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20250115_considerBEdROCK.tif') #20250121 with bedrock 169.8796 total + 149.1977 landcover 
...
#
#NT yellow knife
ca_outline2 <- st_read('/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/2021digitallpr_000b21a_e/lpr_000b21a_e.shp')
NT0 <- ca_outline2 %>% filter(PRNAME == "Northwest Territories / Territoires du Nord-Ouest")
NT84 <- st_transform(NT0,crs = 'epsg:4326')
...
#
# (f_k <- list.files('/mnt/Fastrun/Canada_C/k/all_2100_1231',full.names = T))
(f_k <- list.files('/mnt/Fastrun/Canada_C/k/all_2100_20250109',full.names = T))
...
# lu_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_turnover_rate1981to2000_GBM_20240927.tif')
# cardamom_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_turnover_rate1981to2000_GBM_20240927.tif')
# lu_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_turnover_rate1981to2000_StackE_20241231.tif') #/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/007_1_for_updateSOC2ndtimes/001_1_h2o_automl_build_turnoverrate_model.R
# cardamom_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_turnover_rate1981to2000_StackE_20241231.tif')
lu_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_turnover_rate1981to2000_StackE_20250109.tif') #/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/007_1_for_updateSOC2ndtimes/001_1_h2o_automl_build_turnoverrate_model.R
cardamom_k <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_turnover_rate1981to2000_StackE_20250109.tif')
...
lu_ratio <- clamp(lu_rh0 *1000/litter_base,upper = 5)# this represent spatially rh= litter
lu_ratio1 <- global(lu_rh0*1000 * ca_pure_area84,function(x) sum(x,na.rm = T))/global(litter_base * ca_pure_area84,function(x) sum(x,na.rm = T))

#plot(lu_ratio)
#title(main='lu_to_litter')
card_ratio <- clamp(cardamom_rh0 *1000/litter_base,upper=5)
card_ratio1 <- global(cardamom_rh0 * 1000 * ca_pure_area84,function(x) sum(x,na.rm = T))/global(litter_base * ca_pure_area84,function(x) sum(x,na.rm = T))
#plot(card_ratio)
...
#####
# diff layer from 20_01_M3_initial_diff_layer_create.R
...
card <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_historical_M3_balance_difflayer_StackE_20250117.tif')
## 因为一开始的利用historical 数据预测的Kbase数据和实际的原来的cinput数据有差异，因此需要在一开始把这个差距干掉，并假设这个差距一直有。
lu <- rast('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_historical_M3_balance_difflayer_StackE_20250117.tif')
...
fveg_mods <- c("ACCESS-ESM1-5",'BCC-CSM2-MR','CanESM5','CanESM5-1','CESM2-WACCM','CMCC-CM2-SR5','CMCC-ESM2','EC-Earth3-CC','EC-Earth3-Veg','EC-Earth3-Veg-LR','IPSL-CM6A-LR','KIOST-ESM','NorESM2-LM','NorESM2-MM','TaiESM1')
...
output_ca <- list()
for (km in k_type) {
  # k_base <- rast(paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',km,'_base_turnover_rate1981to2000_GBM_20240927.tif'))
  k_base <- rast(paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',km,'_base_turnover_rate1981to2000_StackE_20250109.tif'))
  soc_s <- list()
  # c_input <- list()
  # c_output <- list()
  socs0 <- vector()
  ...
  for (i in models[model_no]) {
    for (scenario in c('ssp245','ssp585')) { #change c('ssp245','ssp585') to c('ssp585) when 2300
      ...
      #ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20240730.tif')
      # ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20241230.tif')
      # ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20250107.tif') # with no bedrock constrain for data 20250120
      ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20250115_considerBEdROCK.tif') #20250121 with bedrock 169.8796 total + 149.1977 landcover ; for data 20250121
      ...
      (C_inp <- grep(pattern = paste0('litter_PET_CLIMATE_SM_',i,'_',scenario),f_veglit,value=T))
      
      if(km == 'cardamom'){
        ratio00 <- card
        #ratio00 <- card_ratio1$global
      } else {
        ratio00 <- lu
        #ratio00 <- lu_ratio1$global
        }
      
      (f_k0 <- grep(glob2rx(paste0('*',km,'*',i, '_', scenario,'*')), f_k, value = T))
      
      for(k in 1:10){  # all 10 period to 2100
        C_inp_use <- rast(C_inp[k]) #* ratio00 
        #NOTE::: we temporally not multiply intial ratio of rh to litter, as it will increase in the future, though not too much, may several PG, but will increase. if further other ESM model's output showed a sharply drop, then we can multiply a factor. 暂时不乘以ratio00了，因为乘完之后，SOC升高，但高的不是很离谱，如果到时候其它的下降很大，我们可以bias矫正
        C_out_use <- rast(f_k0[k])
        for(j in 1:10){ # each period 10 years 
          cat('\n',k,j,'\n')
          ca_soc1m <- ca_soc1m * (1- C_out_use / 10000) + C_inp_use/1000 - ratio00/1000# g to kg 
          ...
          #
          NT_socs[(k-1) * 10 + j]   <- global(mask(ca_som3m,NT84),function(x) sum(x,na.rm = T))/10^12
          #landcover
          ...
          #peatland from L168 /mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/006_Update_after_EC_CSSS/11_02_method2_pureML_calculation.R
          ...
          #
          NT_peat[(k-1)*10 + j] <- global(mask(ca_peat_stock84_lc,NT84),fun= function(x) sum(x,na.rm=TRUE))/10^14
          # plot(unlist(socs0),type='l',col='maroon',lwd=2.5)
          triger0 <- 10 * (k-1) + j
      ...
      # writeRaster(do.call(c,soc_s),filename = paste0('/mnt/DataSpace/Projects/Canada_C/turnover_constrain/model_outputs/SOC_stock_sasha_2box_balance/',name0,'_',expt,'_based_on_sasa_two_box_balance_2060_2100status_bedrock20250120.tif'),overwrite=TRUE)
      # saveRDS(list(socs0,total_peat_storage84,total_storage84_lc),paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',name0,'_SOC_dynamic_fluxcom_gpp_all6_annual20240826_newlitter_bedrock_20250120.rds'))
      writeRaster(do.call(c,soc_s),filename = paste0('/mnt/DataSpace/Projects/Canada_C/turnover_constrain/model_outputs/SOC_stock_sasha_2box_balance/',name0,'_',expt,'_based_on_sasa_two_box_balance_2060_2100status_bedrock20250121.tif'),overwrite=TRUE)
      saveRDS(list(socs0,total_peat_storage84,total_storage84_lc,NT_socs,NT_peat),paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',name0,'_SOC_dynamic_fluxcom_gpp_all6_annual20240826_newlitter_bedrock_20250121.rds'))
