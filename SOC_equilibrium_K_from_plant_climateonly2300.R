...
ca_soc1m <- rast(
  '/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20240730.tif'
)
...
expt = 'plant_climateonly2300'
models <- c("CanESM5" ,
            "EC-Earth3-Veg",
            "IPSL-CM6A-LR",
            "MRI-ESM2-0",
            "ACCESS-ESM1-5")
model_plant_2300 <- c("CanESM5", "IPSL-CM6A-LR", "ACCESS-ESM1-5")
models <- intersect(models, model_plant_2300)
...
#
f_k <- list.files(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/plant_climateonly_585_2300',
  full.names = T
)
...
lu_k <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_turnover_rate1981to2000_GBM_20240730.tif'
)
cardamom_k <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_turnover_rate1981to2000_GBM_20240730.tif'
)
...
(
  f_veglit <- list.files(
    pattern = 'tif',
    pat = '/mnt/Fastrun/Canada_C/litter_climate_only2300/',
    full.names = T
  )
)
...
for (km in k_type) {
  k_base <- rast(
    paste0(
      ...
      km,
      '_base_turnover_rate1981to2000_GBM_20240730.tif'
    )
  )
  soc_s <- list()
  # c_input <- list()
  ...
  for (i in models[model_no]) {
    for (scenario in c('ssp585')) {
      ...
      (C_inp <- grep(
        pattern = paste0('climateonly2300_', i, '_', scenario),
        f_veglit,
        value = T
      ))
      if (km == 'cardamom') {
      ...
      for (k in 1:20) {
        #as there is only 20 which represent 2100-2300
        # from 2100 to 2300 as in climateonly2100 has already include ssp585 before 2100, and there is only 20 for C_input
        # from 2100 to 2300 as in climateonly2100 has already include ssp585 before 2100, and there is only 20 for C_input
        ...
        for (j in 1:10) {
          ...
          total_peat_storage84[(k - 1) * 10 + j] <- total_peat_C84 * 10^7 /
            10^15 / 10^6
          triger0 <- 10 * (k - 1) + j + 100
          if (triger0 %in% c(200, 300)) {
            # if you want to output 20 40 60 80 then replace the values.
            ind1 <- which(c(200, 300) == triger0)
      ...
      writeRaster(
        ...
        filename = paste0(
          ...
          '_',
          expt,
          '_based_on_sasa_two_box_balance2100_2300.tif'
        ),
        overwrite = TRUE
      )
      saveRDS(
        ...
        paste0(
          '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',
          name0,
          '_SOC_dynamic_fluxcom_gpp_kfromplantclimateonly2100_2300_annual20240826_newlitter.rds'
        )
      )
    }
  }
