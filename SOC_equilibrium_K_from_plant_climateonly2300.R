...
(f_veglit <- list.files(pattern = 'tif', pat = '/mnt/Fastrun/Canada_C/litter_climate_only2300/',full.names = T))
...
for (km in k_type) {
  ...
  for (i in models[model_no]) {
    for (scenario in c('ssp585')) { #change c('ssp245','ssp585') to c('ssp585) when 2300
      ...
      (C_inp <- grep(pattern = paste0('climateonly2300_',i,'_',scenario),f_veglit,value=T))
      if(km == 'cardamom'){
        ratio00 <- card_ratio
      ...
      for(k in 1:20){  
        ...
        for(j in 1:10){ # each period 10 years 
          ...
          if(triger0 %in% c(200,300)){  # if you want to output 20 40 60 80 then replace the values. 
      ...
      writeRaster(do.call(c,soc_s),filename = paste0('/mnt/DataSpace/Projects/Canada_C/turnover_constrain/model_outputs/SOC_stock_sasha_2box_balance/',name0,'_',expt,'_based_on_sasa_two_box_balance2100_2300.tif'),overwrite=TRUE)
      saveRDS(list(socs0,total_peat_storage84,total_storage84_lc),paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',name0,'_SOC_dynamic_fluxcom_gpp_kfromplantclimateonly2100_2300_annual20240826_newlitter.rds'))
    }
  }
}
