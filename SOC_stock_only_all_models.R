...
system.time(ca_base_new <- fread('/mnt/Fastrun/Canada_C/ca_base_new_df20240730.csv')) # newly created from 20241230
...
lt1000 <- function(x) {
  x[which(x > 120.3014)] <- 120.3014  # som to soc log10(580) ; 1000/1.724
  return(x)
}
...
models00 <- model_all[begin:end] # for parallel 
...
for (i in models00) {
  for(scenario in ss0) {
    ...
    # for (k in 2:5) {
      ...
      #20240329 changed to k + 1 , and length(f_plant)/3 to length(f_plant)/2
      # f_ind <- f_plant[seq(k + 1, length(f_plant), length(f_plant) / 2)] #time period
      ...
      for (j in 1:8) {
        ...
        #20240730 /mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/006_Update_after_EC_CSSS/001_h2o_automl_build_SOC_model.R
        ...
        stack_m <- h2o.loadModel("/mnt/DataSpace/Projects/Canada_C/models_after_EC/bestfamiliy_ensemble/StackedEnsemble_AllModels_4_AutoML_1_20241228_120929") # 比较慢
        ...
        maps <- pred_fun0(model=stack_m,t_data=test)
        #maps0 <- do.call(c, maps)
        maps1 <- terra::clamp(maps, upper = 120.3014, values = T)
        maps2 <- round(exp(sqrt(maps1))/100,2)
        names(maps2) <- 'Stack'
        # terra::writeRaster(maps2,
        #                    paste0('/mnt/Fastrun/Canada_C/supplementary_plant_pet_climate_sm_0830/',dep00[j],'_','PLANT_PET_CLIMATE_SM_',i,'_',scenario,'_',time00[k],'.tif'),overwrite = TRUE)
        # 
        terra::writeRaster(maps2,
        paste0('/mnt/Fastrun/Canada_C/supplementary_plant_pet_climate_sm_1230/',dep00[j],'_','PLANT_PET_CLIMATE_SM_',i,'_',scenario,'_',time00[k],'.tif'),overwrite = TRUE)
      }
      h2o.shutdown(prompt = FALSE)
    }
  }}
