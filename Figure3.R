...
quadrotic_data <- readRDS(
  '/mnt/DataSpace/Projects/Canada_C/processed_output/CMIP6_LAI_to_GDD5_and_Observation_relationship_removeland15_ssp245.rds'
)[[3]]#1 quadrotic 2 gaussian 3 sigmoid 4 exp
quadratic_df <- do.call(rbind, quadrotic_data)
quadratic_df$model <- factor(
  quadratic_df$model,
  levels = c(
    'ACCESS-ESM1-5',
    'BCC-CSM2-MR',
...
cols <- c(colorRampPalette(brewer.pal(8, 'Set1'))(length(unique(quadratic_df$model)) -
                                                    1), 'black')
...
cols <- setNames(c(colorRampPalette(brewer.pal(10, 'Paired'))(length(model_all) -
                                                                1), 'black'), model_all)
...
pgdd <-
  ...
  scale_y_continuous(limits = c(0, 4.5), sec.axis = dup_axis()) +
  theme_bw() + theme(
    legend.position = "right",
...
##### fig 3b
slope00 <- readRDS(
  '/mnt/DataSpace/Projects/Canada_C/processed_output/CMIP6_LAI_to_GDD5_and_Observation_relationship_slopemax_removeland15_ssp245.rds'
)[[3]]
...
mean(c(
  slope01$Slopemax[c(1, 2, 5, 11, 12)],
  mean(slope01$Slopemax[c(3, 4)]),
  mean(slope01$Slopemax[c(6:8)]),
  mean(slope01$Slopemax[c(9, 10)])
))
...
slope_fig3b <-
  ...
  labs(
    x = ' ',
    y = expression(Max ~ omega ~ " (" * "×" * 10^-3 ~ m^2 ~ m^-2 ~ degree *
                     C^-1 * ")"),
    col = 'ESM',
    shape = 'ESM'
  ) +
  guides(col = guide_legend(ncol = 3), linetype = guide_legend(ncol = 3)) +
...
#######fig 3c
df_a <- readRDS(
  '/mnt/DataSpace/Data_pool/CMIP6_cSoil_trend/csoil_rh_turnover_dataframe_withslopes_20250601.rds'
)
csoil00 <- c(
  'ACCESS-ESM1-5',
  'BCC-CSM2-MR',
...
Q10s <- data.frame(
  Q10_W = Q10_values_w,
  Q10_C = Q10_values_c,
  model = c(csoil00[c(1, 3:11)], 'Observation')
) %>%
  pivot_longer(
    data = .,
...
logKK3 <-
  ...
  annotate(
    'text',
    label = c('Warmer', 'Colder'),
    x = c(1 / 278.15, 1 / 268.15),
    y = -6.25,
    size = 8,
    family = 'Times'
  ) +
  ...
  labs(
    y = expression("ln k (yr"^{
      -1
    } * ")"),
    x = expression("Inverse mean annual temperature (1/T, K"^{
      -1
    } * ")"),
    col = 'ESM',
    shape = 'ESM',
...
head(Q10s)
Q10s <- Q10s %>% mutate(
  Source = c(rep('ESM', 20), rep('Observation', 2)),
...
Q10_fig3d <-
  ...
  scale_color_manual(values = cols) +
  scale_linetype_manual(values = linetypes) +
  scale_x_discrete(breaks = c('Q10_C', 'Q10_W'),
...
logKK3
Q10_fig3d + theme(axis.text = element_text(
  face = 'bold',
  family = 'Times',
  size = 16
))
...
legend_df <- data.frame(
  x = rnorm(n = length(model_all) * 10),
  y = rnorm(n = length(model_all) * 10),
  model = factor(model_all, levels = model_all)
)
legend_fig3 <-
  ggplot() +
  geom_point(data = legend_df, aes(
    x = x,
    y = y,
    shape = model,
    linetype = model
  )) +
  geom_line(data = legend_df, aes(
    x = x,
    y = y,
    col = model,
    shape = model,
...
combined_cow3
ggsave(
  filename = '/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/figures/Figures_new/fig3_new.tiff',
  width = 12.5,
  height = 8.5,
  units = 'in',
  dpi = 600,
...
ggsave(
  filename = '/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/figures/Figures_new/fig3_new.pdf',
  width = 12.5,
  height = 8.5,
  units = 'in',
  bg = 'white'
