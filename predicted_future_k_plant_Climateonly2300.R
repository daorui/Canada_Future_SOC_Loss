...
#edit  from /mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/002_code/09_turnover_k_PLANT_PET_climate_sm_6.R
...
model_plant_climate2300 <- c('CanESM5','IPSL-CM6A-LR','ACCESS-ESM1-5')
...
models <- model_plant_climate2300[begin:end] # for parallel 
...
for (i in models) {
  for(scenario in c('ssp585')) {
    ...
    for (k in 1:30) {  # as pet always 2100
      ...
      map12 <- rast(f_map[k])[[12]] # original extend, unfortunityly base data csv using current ext with ca_shp2 (not from world) + 1
      ...
      # add bed rock depth 
      model000 <- grep(pattern = glob2rx("*GBM*20240927*"), x = list.files(path = paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/traind_model/',mk),full.names = T),value=T)
      ...
      #                    paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/plant_climateonly_585_2300/',mk,'_','plant_climate_2300_',i,'_',scenario,'_',time00[k],'.tif'),overwrite = TRUE)
      terra::writeRaster(maps2,
                         paste0('/mnt/Fastrun/Canada_C/k/plant_climateonly_585_2300/',mk,'_','plant_climate_2300_',i,'_',scenario,'_',time00[k],'.tif'),overwrite = TRUE)
