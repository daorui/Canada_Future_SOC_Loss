...
# national
cmip6_soc <- readRDS(
  '/mnt/DataSpace/Data_pool/CMIP6_cSoil_trend/anomaly_in_future_litter_csoil_totalsoc.rds'
)
# peatland
...
#
df_common_box <- df_commom_family %>% filter(time == 2100) %>% mutate(scen = case_when(scenario == 'SSP245' ~
                                                                                         2110, scenario == 'SSP585' ~ 2120))

# family weighted mean
df_commom_ave_family_ave <- df_commom_family %>%
  group_by(scenario, time) %>%
  summarize(
    upper = mean(anomaly) + 1.645 * sd(anomaly),
    lower = mean(anomaly) - 1.645 * sd(anomaly),
    Totalsoc = mean(Totalsoc),
    upper1 = max(anomaly),
    lower1 = min(anomaly),
...
model1 <- unlist(lapply(model_f, function(x)
  grep(x, model0, value = T)))
...
theme0 <- theme(
  ...
  axis.text.x.bottom = element_text(face = 'bold', margin =
                                      margin(t = 10, 0, 0, 0)),
  ...
  legend.box.just = "top",
  legend.text = element_text(family = 'Times', size = 4 *
                               2.5),
...
p1 <-
  ...
  scale_fill_manual(
    name = 'Scenario',
    values = c('SSP245' = '#92c5de', 'SSP585' = '#fcd1c5'),
    labels = c('SSP245' = 'SSP2-4.5', 'SSP585' = 'SSP5-8.5'),
  ) +
  scale_color_manual(
    name = 'Scenario',
    values = c('SSP245' = '#70a0cd', 'SSP585' = '#990002'),
    labels = c('SSP245' = 'SSP2-4.5', 'SSP585' = 'SSP5-8.5')
  ) +
  scale_shape_manual(
    name = 'ESMs',
  ...
  guides(shape = guide_legend(
    ...
    # shape size
    theme = theme(legend.title.position = "top", legend.byrow = TRUE)
  )) +
...
p10 <-
  ...
  geom_boxplot(
    data = df_common_box,
    aes(x = scen, y = anomaly, fill = scenario),
    outlier.shape = NA,
    alpha = 0.7,
    size = 0.3
  ) +
...
soc_stock_use <- readRDS(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/Processed_output/method2_pureml_change_pattern_landcover/temporal_trend_2025_national.rds'
)
...
#
m2_df_box <- m2_df_family %>% filter(time == 2100) %>% mutate(scen = case_when(scenario == 'SSP245' ~
                                                                                 2110, scenario == 'SSP585' ~ 2120))
...
shape_value = c()
get_model <- function(y) {
  unlist(lapply(model_f, function(x)
    grep(x, y, value = T)))
}
if (exp0 == 'All') {
  model1 <- get_model(model_all)
} else {
  model1 <- get_model(model_clim2100)
}
if (exp0 == 'All') {
  delta245 = -1.08
  delta585 = -1.4
...
p2 <-
  ...
  scale_x_continuous(
    ...
    limits = c(1995, 2130),
    expand = c(0, 0)
  ) + # not expand， set the xlim exact at 1995 投2130 for segment dash
  labs(y = 'SOC storage change (Pg)', x = 'Year') +
  geom_line(
    data = m2_df_ave_family_ave,
    aes(
...
get_data <- function(ye, model, data_file, resp, scen) {
  if (ye == 2100) {
    ...
    exp0 <- get_exp(model, ssp1 = NULL, ssp2 = 'ssp585')
  }
  if (forpeat) {
    data0 <- data.frame(do.call(cbind, lapply(exp0, function(x)
      unlist(
        readRDS(grep(x, data_file, value = T))[[2]]
      )))[, grep(glob2rx(paste0(resp, '*', scen)), exp0)])
  } else {
    data0 <- data.frame(do.call(cbind, lapply(exp0, function(x)
      unlist(
        readRDS(grep(x, data_file, value = T))[[1]]
      )))[, grep(glob2rx(paste0(resp, '*', scen)), exp0)])
  }
...
f1 <- list.files(
  glob2rx('*annual20240826_newlitter_bedrock*0121.rds$'),
  path = '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs',
  full.names = T
)
model_plant_clim2100 <- c(
  'CanESM5',
  'EC-Earth3-Veg',
  'INM-CM4-8',
  'IPSL-CM6A-LR',
...
m3_clim2100_plant_ca_ssp245_lu <- get_data(
  ye = 2100,
  model = model_plant_clim2100,
  data_file = m3_clim2100_plant,
  resp = 'lu',
  scen = 'ssp245'
) %>% mutate(Rh = 'Lu',
             scenario = 'SSP245',
             time = 2001:2100)
m3_clim2100_plant_ca_ssp585_lu <- get_data(
  ye = 2100,
  model = model_plant_clim2100,
  data_file = m3_clim2100_plant,
  resp = 'lu',
  scen = 'ssp585'
             ...
             scenario = 'SSP585',
             time = 2001:2100)
m3_clim2100_plant_ca_ssp245_card <- get_data(
  ye = 2100,
  model = model_plant_clim2100,
  data_file = m3_clim2100_plant,
  resp = 'cardamom',
  scen = 'ssp245'
             ...
             scenario = 'SSP245',
             time = 2001:2100)
m3_clim2100_plant_ca_ssp585_card <- get_data(
  ye = 2100,
  model = model_plant_clim2100,
  data_file = m3_clim2100_plant,
  resp = 'cardamom',
...
m3_df_245_1 <- m3_clim2100_plant_ca_ssp245_lu %>% mutate(exp = 'Clim2100') %>% tidyr::pivot_longer(col = starts_with('lu'),
                                                                                                   names_to = 'experiment',
                                                                                                   values_to = 'SOC_stock')
m3_df_585_2 <- m3_clim2100_plant_ca_ssp585_lu %>% mutate(exp = 'Clim2100') %>% tidyr::pivot_longer(col = starts_with('lu'),
                                                                                                   names_to = 'experiment',
                                                                                                   values_to = 'SOC_stock')
m3_df_245_3 <- m3_clim2100_plant_ca_ssp245_card %>% mutate(exp = 'Clim2100') %>% tidyr::pivot_longer(col = starts_with('card'),
                                                                                                     names_to = 'experiment',
                                                                                                     values_to = 'SOC_stock')
m3_df_585_4 <- m3_clim2100_plant_ca_ssp585_card %>% mutate(exp = 'Clim2100') %>% tidyr::pivot_longer(col = starts_with('card'),
                                                                                                     names_to = 'experiment',
                                                                                                     values_to = 'SOC_stock')
...
m3_df_family <- rbind(m3_df_245_1, m3_df_585_2, m3_df_245_3, m3_df_585_4) %>%
  ...
  #mutate(family = substr(model,1,4),anomaly = SOC_stock - 152.8529) %>% #before bedrock 160.753
  {
    if (peat000) {
      mutate(.,
             family = substr(model, 1, 4),
             anomaly = SOC_stock - 41.93237) # M3 base from /mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/007_1_for_updateSOC2ndtimes/12_02_visual_combine_spatial2_2025new.R
    } else {
      mutate(.,
             family = substr(model, 1, 4),
             anomaly = SOC_stock - 149.1977)
    }
  } %>%
  #peat
...
#
m3_df_box <- m3_df_family %>% filter(time == 2100, Rh == 'Cardamom') %>% mutate(scen = case_when(scenario == 'SSP245' ~
                                                                                                   2110, scenario == 'SSP585' ~ 2120))
...
p3 <-
  ...
  geom_ribbon(
    data = m3_df_ave_family_ave %>% filter(Rh == 'Cardamom'),
    aes(
      x = time,
      ymin = lower,
      ymax = upper,
      group = interaction(Rh, scenario),
      fill = scenario
    ),
    alpha = 0.6
  ) +
  ...
  (
    p20 + labs(x = "") +
      ...
      theme(
        ...
        axis.text.y.right = element_blank(),
        axis.ticks.y.right = element_blank(),
        plot.tag = element_text(hjust = -20, vjust = -2)
      ) +
      scale_x_continuous(
        breaks = c(seq(2000, 2100, 25)),
        labels = c(2000, '', 2050, '', 2100),
        limits = c(1995, 2130),
        expand = c(0, 0),
      ...
      ggtitle("")
  ) + theme(axis.text.x.bottom = element_text(margin = margin(t = 4, unit = 'pt'))) +
  (
    p30 + labs(y = "") +
      guides(
        shape = guide_none(),
...
ggsave(
  plot = fig1,
  filename = paste0(path0, 'Fig1_temporal_trend.tiff'),
  width = 15,
  height = 6,
  units = 'in',
  dpi = 600,
...
pdf(
  file = paste0(path0, "fig1_temporal_trend.pdf"),
  width = 15,
  height = 6
)
dev.off()
