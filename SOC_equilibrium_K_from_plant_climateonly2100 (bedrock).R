...
# ca_soc1m <- rast('/mnt/DataSpace/Projects/Canada_C/processed_output/EC_after_CSSS/canada_SOCs_1m_kgm2_expoential_relationship_SOC_BD_with_originaldata_20250107.tif')
...
model_plant <- c('ACCESS-ESM1-5','BCC-CSM2-MR','CanESM5','CanESM5-1','EC-Earth3-CC','EC-Earth3-Veg','EC-Earth3-Veg-LR','INM-CM4-8','INM-CM5-0','IPSL-CM6A-LR')
models <- intersect(models,model_plant)
...
f_k <- list.files('/mnt/Fastrun/Canada_C/k/plant_climateonly_2100_20250109',full.names = T)
...
#20240423  # diff layer from 20_01_M3_initial_diff_layer_create.R
...
(f_veglit <- list.files(pattern = 'tif', pat = '/mnt/DataSpace/Projects/Canada_C/SOC_and_k_fromFastrun/litter_climate_only2100/',full.names = T))
...
for (km in k_type) {
  ...
  for (i in models[model_no1:model_no2]) {
    for (scenario in c('ssp245','ssp585')) { #change c('ssp245','ssp585') to c('ssp585) when 2300
      ...
      saveRDS(list(socs0,total_peat_storage84,total_storage84_lc),paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',name0,'_SOC_dynamic_fluxcom_gpp_kfromplantclimateonly2100_annual20240826_newlitter_bedrock20250121.rds'))
    }
  }
}
