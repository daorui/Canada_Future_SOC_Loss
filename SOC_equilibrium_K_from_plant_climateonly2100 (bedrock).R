...
) #20250121 with bedrock 169.8796 total + 149.1977 landcover ; for data 20250121
...
f_k <- list.files('/mnt/Fastrun/Canada_C/k/plant_climateonly_2100_20250109',
                  full.names = T)
...
#20240423  #
card <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_historical_M3_balance_difflayer_StackE_20250117.tif'
)
lu <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_historical_M3_balance_difflayer_StackE_20250117.tif'
)


#20240826
litter_base <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/litterinput_base_litterinput_1981to2000_GBM_20240824.tif'
)
#
...
lu_k <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/lu_base_turnover_rate1981to2000_StackE_20250109.tif'
) #/mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/007_1_for_updateSOC2ndtimes/001_1_h2o_automl_build_turnoverrate_model.R
cardamom_k <- rast(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/cardamom_base_turnover_rate1981to2000_StackE_20250109.tif'
)
...
(
  f_veglit <- list.files(
    pattern = 'tif',
    pat = '/mnt/DataSpace/Projects/Canada_C/SOC_and_k_fromFastrun/litter_climate_only2100/',
    full.names = T
  )
)
...
for (km in k_type) {
  ...
  k_base <- rast(
    paste0(
      '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',
      km,
      '_base_turnover_rate1981to2000_StackE_20250109.tif'
    )
  )
  soc_s <- list()
  # c_input <- list()
  ...
  for (i in models[model_no1:model_no2]) {
    for (scenario in c('ssp245', 'ssp585')) {
      #change c('ssp245','ssp585') to c('ssp585) when 2300
      ...
      (C_inp <- grep(
        pattern = paste0('climateonly2100_', i, '_', scenario),
        f_veglit,
        value = T
      ))
      if (km == 'cardamom') {
      ...
      for (k in 1:10) {
        ...
        for (j in 1:10) {
          ...
          #TOTAL STORAGE
          total_peat_C84 <- global(
            ca_peat_stock84_lc,
            fun = function(x)
              sum(x, na.rm = TRUE)
          )
          ####
          total_peat_storage84[(k - 1) * 10 + j] <- total_peat_C84 * 10^7 /
            10^15 / 10^6
          #
          triger0 <- 10 * (k - 1) + j
          if (triger0 %in% c(60, 100)) {
            # if you want to output 20 40 60 80 then replace the values.
      ...
      writeRaster(
        do.call(c, soc_s),
        filename = paste0(
          '/mnt/DataSpace/Projects/Canada_C/turnover_constrain/model_outputs/SOC_stock_sasha_2box_balance/',
          name0,
          '_',
          expt,
          '_based_on_sasa_two_box_balance_2060_2100status_bedrock20250121.tif'
        ),
        overwrite = TRUE
      )
      saveRDS(
        list(socs0, total_peat_storage84, total_storage84_lc),
        paste0(
          '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',
          name0,
          '_SOC_dynamic_fluxcom_gpp_kfromplantclimateonly2100_annual20240826_newlitter_bedrock20250121.rds'
        )
      )
    }
  }
