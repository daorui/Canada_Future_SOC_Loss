...
future_period <- data.frame(stardyear = c(1970, seq(2001, 2100, 10)),
                            endyear = c(2000, seq(2010, 2100, 10)))

# calculate annual mean value of outputs
models_output <- function (var0 , model0 , time0) {
  ind00 <- grep(pattern = paste0('.*', var0, '_.*', model0, '_.*', 'historical'),
                x = t1)
  ind01 <- grep(pattern = paste0('.*', var0, '_.*', model0, '_.*', time0),
                x = t1)
  ...
  for (k in m) {
    ...
    if (year(times_span0)[1] <= startdate) {
      ...
      #
      if (var0 == 'pr') {
        f40 <- f3 * 86400 * mon_days_ind + 0.1
      } else {
        f40 <- f3 - 273.15
      }
      all_ind <- which(1:nrow(future_period) == k)
      annual_month_average[[all_ind]] <- tapp(x = f40,
                                              index = 1:12,
                                              fun = 'mean')  #
    }
...
for (ii in 1:nrow(all_combin)) {
  ind0000 <- grep(
    pattern = paste0('.*', all_combin[ii, 2], '_.*', all_combin[ii, 1], '_.*', all_combin[ii, 3]),
    x = t1
  )
  f20 <- rast(t1[ind0000])
  ...
  assign(
    do.call(paste, c(
      all_combin[ii, ], min(times_span00), max(times_span00), sep = '_'
    )) ,
    models_output(
      var0 = all_combin[ii, 2],
      model0 = all_combin[ii, 1],
      time0 = all_combin[ii, 3]
    )
  )
}
...
for (i in  1:length(model)) {
  for (j in 1:4) {
    for (k in 2:3) {
      hist00 <- grep(
        pattern = paste0(model[i], "_", var00[j], "_", 'historical'),
        x = ls(),
        value = TRUE
      )
      future00 <- grep(
        pattern = paste0(model[i], "_", var00[j], "_", time00[k]),
        x = ls(),
        value = TRUE
      )
      if (var00[j] == 'pr') {
        assign(paste0(paste0(
          model[i], "_", var00[j], "_", time00[k], "_", "bias"
        )), lapply(
          X = get(future00),
          FUN = function(x) {
            (x - get(hist00)) / (get(hist00) + 0.1)
          }
        )) # 
      } else {
        assign(paste0(paste0(
...
for (i in all_bias) {
  terra::writeRaster(
    get(i),
    paste0('/mnt/DataSpace/Data_pool/02climate_bias_10/', i, '.tif'),
    overwrite = TRUE
  )
}
...
a0 <- list.files(
  pattern = 'bias.tif$',
  '/mnt/DataSpace/Data_pool/02climate_bias_10',
  full.names = TRUE
)
a_0 <- list.files(
  pattern = 'tas_.*.tif$',
...
for (i in 1:length(a_file0)) {
  ...
  writeRaster(
    wd5bias1,
    filename = paste0(
      '/mnt/DataSpace/Data_pool/02_1climate_bias5min_10/0',
      basename(a_file0[i])
    ),
    overwrite = T
  )
...
biovariables00 <- function (x) {
  ...
  if (nrow(x) == 12)
    x <- rbind(x, x[1:2, ])
  x[, 4] <- (x[, 2] + x[, 3]) / 2
  x[1:12, 5] <- caTools::runmean(x[, 1],
                                 3,
                                 alg = "fast",
                                 endrule = "trim",
                                 align = "left") * 3
  x[1:12, 6] <- caTools::runmean(x[, 4],
                                 3,
                                 alg = "fast",
                                 endrule = "trim",
                                 align = "left")
...
pr_hist0 <- list.files(
  pattern = 'tif',
  path = paste0('/mnt/File0/DAAATAAA/world_climate/5mins/wc2.1_5m_', var2[1]),
  full.names = TRUE
)
tmax_hist0 <- list.files(
  pattern = 'tif',
  path = paste0('/mnt/File0/DAAATAAA/world_climate/5mins/wc2.1_5m_', var2[2]),
  full.names = TRUE
)
tmin_hist0 <- list.files(
  pattern = 'tif',
  path = paste0('/mnt/File0/DAAATAAA/world_climate/5mins/wc2.1_5m_', var2[3]),
  full.names = TRUE
)
#}
library(terra)
...
for (j in 2:length(model)) {
  for (k in 1) {
    f_future_bias <- list.files(
      pattern = paste0(model[j], '_.*', scaro[k], '.*tif$'),
      path = '/mnt/DataSpace/Data_pool/02_1climate_bias5min_10',
      full.names = TRUE
    )
    ...
    for (i in 1:10) {
      pr_future1 <- (abs(pr_future[[ind03[i, 1]:ind03[i, 2]]]) + 1) * pr_hist1 +
        0.1
      tmax_future1 <- tmax_future[[ind03[i, 1]:ind03[i, 2]]] + tmax_hist1
      tmin_future1 <- tmin_future[[ind03[i, 1]:ind03[i, 2]]] + tmin_hist1
      futur_bc_list <- terra::app(
        x = c(pr_future1, tmax_future1, tmin_future1),
        fun = biovariables00,
        cores = 32
      )#
      nam1 <- rowMeans(data.frame(
        s1 = seq(2000, 2090, 10),
        e1 = seq(2010, 2100, 10)
      ))[i]
      ...
      t1 <- disagg(
        ...
        method = 'bilinear',
        filename = paste0(
          '/mnt/DataSpace/Data_pool/03cmip6_processed_bio19_5min_10/',
          model[j],
          "_",
          scaro[k],
          "_",
        ...
        overwrite = TRUE,
        gdal = c('COMPRESS=LZW', 'ZLEVEL=9', 'NUM_THREADS=64'),
        tempdir = '/mnt/temp4r/temp'
      )
      rm(t1, t9)
      gc()
      gc()
...
future_period <- data.frame(stardyear = c(1970, seq(2001, 2300, 10)),
                            endyear = c(2000, seq(2010, 2300, 10)))
#NOTE: start from 2100
model_2300 <- c("CanESM5",
                "EC-Earth3-Veg",
                "IPSL-CM6A-LR",
                "MRI-ESM2-0",
                "ACCESS-ESM1-5")
...
a_file1 <- do.call(c, lapply(model_2300, function(x)
  grep(paste0(x, '_'), a_file0, value = T)))
temp1 <- '/mnt/File0/DAAATAAA/world_climate/5mins/wc2.1_5m_prec/wc2.1_5m_prec_01.tif'
tmplt <- rast(temp1)
  ...
  for (k in 1) {
    ...
    for (i in 1:30) {
      ...
      t1 <- disagg(
        ...
        overwrite = TRUE,
        gdal = c('COMPRESS=LZW', 'ZLEVEL=9', 'NUM_THREADS=16'),
        tempdir = '/mnt/temp4r/temp'
      )
      rm(t1, t9)
      gc()
...
library(terra)
cmip6_ca <- list.files(
  pattern = 'ssp585.*2100_CA.tif$',
  path = '/mnt/DataSpace/Data_pool/03cmip6_processed_bio19_5min/',
  recursive = F,
  full.names = TRUE
)
...
ni <- function(x) {
  ...
  writeRaster(
    round(t1[[x]], 1),
    filename = paste0(
      '/mnt/DataSpace/Data_pool/04_bios_variable_10/BIO',
      ceiling(x %% 19.1),
      '_',
      gsub('2020_2100_CA.tif', '', basename(sources(t1))),
      begin,
      '-',
      end,
      '.tif'
...
#    function
et_pm <- function(Delta = SlpSVPC(T_mean),
...
for (i in 1:length(mod_use)) {
  ...
  historical_pet0 <- app(tapp(
    historical_pet[[hist_ind]],
  ...
  ssp245_pet0 <- tapp(
    tapp(ssp245_pet[[ssp_ind]], index = rep(1:(
      nlyr(ssp245_pet[[ssp_ind]]) / 12
    ), rep(
      12, nlyr(ssp245_pet[[ssp_ind]]) / 12
    )), fun = sum),
    index = rep(1:(length(ssp_ind) / (10 * 12)), rep(10 , length(ssp_ind) /
                                                       (10 * 12))),
    fun = mean
  )
  ssp585_pet0 <- tapp(
    tapp(ssp585_pet[[ssp_ind]], index = rep(1:(
      nlyr(ssp585_pet[[ssp_ind]]) / 12
    ), rep(
      12, nlyr(ssp585_pet[[ssp_ind]]) / 12
    )), fun = sum),
    index = rep(1:(length(ssp_ind) / (10 * 12)), rep(10 , length(ssp_ind) /
                                                       (10 * 12))),
    fun = mean
  ...
  #
  if (ext(historical_pet0)[2] > 200) {
    historical_pet0 <- rotate(historical_pet0)
  }
  if (ext(ssp245_pet0)[2] > 200) {
    ssp245_pet0 <- rotate(ssp245_pet0)
  }
  if (ext(ssp585_pet0)[2] > 200) {
    ssp585_pet0 <- rotate(ssp585_pet0)
  }
  
  ssp245_bc <- crop(ssp245_pet0 / historical_pet0, tdaye)
  ssp585_bc <- crop(ssp585_pet0 / historical_pet0, tdaye)
  #
  diff_ssp245_1km <- resample(ssp245_bc, tdaye, method = 'bilinear', threads =
                                TRUE)
  diff_ssp585_1km <- resample(ssp585_bc, tdaye, method = 'bilinear', threads =
                                TRUE)
  #
  ...
  writeRaster(
    historical_pet,
    filename = paste0(
      '/mnt/DataSpace/Data_pool/01PET_cmip/Processed_cmip6_pet_10/',
      mod_use[i],
      '_',
      'historical',
      '_',
      year(min(time(ssp245_pet))),
      '-',
      year(max(time(ssp245_pet))),
      '.tif'
    )
  )
  writeRaster(
    ...
    filename = paste0(
      ...
      'ssp245',
      '_',
      year(min(time(ssp585_pet))),
      '-',
      year(max(time(ssp585_pet))),
      '.tif'
    )
  )
  writeRaster(
  ...
  writeRaster(
    future_ssp245,
    filename = paste0(
      '/mnt/DataSpace/Data_pool/02pet_use_10/',
      mod_use[i],
      '_',
      'ssp245',
      '_',
...
for (i in 1:length(ff0)) {
  ...
  for (j in 1:nlyr(t1)) {
    writeRaster(
      round(t1[[j]], 0),
      filename = paste0(
        '/mnt/DataSpace/Data_pool/03pet_use_individual_10/',
        'pet_',
        ff0_b1[i],
        '_',
        time00[j],
...
mod1 <- unique(unlist(lapply(strsplit(mrsos0, '/|_'), function(x)
  x[10])))
mod2 <- unique(unlist(lapply(strsplit(mrso0, '/|_'), function(x)
  x[10])))

mod3 <- intersect(mod1, mod2)

tt1 <- data.frame(model = unlist(lapply(strsplit(mrsos0, '/|_'), function(x)
  x[10])), period = unlist(lapply(strsplit(mrsos0, '/|_'), function(x)
    x[11]))) %>% mutate(mod_period = paste0(model, '-', period))

tt4 <- table(tt1$model[unlist(lapply(unique(tt1$mod_period), function(x)
  which(tt1$mod_period == x)[1]))])
w_top <- tt4[which(tt4 == 3)]

tt2 <- data.frame(model = unlist(lapply(strsplit(mrso0, '/|_'), function(x)
  x[10])), period = unlist(lapply(strsplit(mrso0, '/|_'), function(x)
    x[11]))) %>% mutate(mod_period = paste0(model, '-', period))

tt3 <- table(tt2$model[unlist(lapply(unique(tt2$mod_period), function(x)
  which(tt2$mod_period == x)[1]))])
...
get_bc <- function(model, scenario) {
  ...
  pure_hist0 <- rast(grep(
    paste0(model, '_historical'),
    grep('mrsos_', f1, value = T),
    value = T
  )) #
  pure_hist_all <- rast(grep(
    paste0(model, '_historical'),
    grep('mrso_', f1, value = T),
    value = T
  )) #
  ...
  #
  if (xmax(pure_hist0) > 200) {
    pure_hist_t2 <- rotate(pure_hist0)
  } else {
    pure_hist_t2 <- pure_hist0
  }
  if (xmax(pure_hist1) > 200) {
    pure_hist_t2_2 <- rotate(pure_hist1)
  } else {
    pure_hist_t2_2 <- pure_hist1
  }
  ...
  if (model == 'BCC-CSM2-MR') {
    time(hist0) <- seq(as.Date('2015-01-16'), as.Date('2100-12-16'), by = 'month')
  }
  if (model == 'BCC-CSM2-MR') {
    time(hist1) <- seq(as.Date('1850-01-16'), as.Date('2014-12-16'), by = 'month')
  }
  #
  if (xmax(hist0) > 200) {
  ...
  if (xmax(hist1) > 200) {
    hist_t2_2 <- rotate(hist1)
  } else {
    hist_t2_2 <- hist1
  }
  
  hist_ind <- which(time(hist_t2) %in% seq(as.Date('2016-01-01'), as.Date('2021-12-31'), by =
                                             'day'))
  ...
  hist_ind_2 <- which(time(hist_t2_2) %in% seq(as.Date('2016-01-01'), as.Date('2021-12-31'), by =
                                                 'day'))
  hist_t3_2 <- hist_t2_2[[hist_ind_2]] # 
  hist_t4_2 <- app(tapp(hist_t3_2, index = rep(1:(
    length(hist_ind_2) / 12
  ), rep(
    12, length(hist_ind_2) / 12
  )), fun = sum), fun = mean)
  
  #
  #
  f_ssp <- grep(paste0(model, '_', scenario),
                grep('mrsos_', f1, value = T),
                value = T)
  ...
  if (!is.null(diff_ssp)) {
    lapply(1:nlyr(f_ssp_1km), function(x)
      writeRaster(
        f_ssp_1km[[x]],
        filename = paste0(
          '/mnt/DataSpace/Data_pool/05_smap_10/',
          'SMAP',
          '_',
          model,
          '_',
  ...
  f_ssp_1 <- grep(paste0(model, '_', scenario),
                  grep('mrso_', f1, value = T),
                  value = T)
  ...
  if (length(f_ssp_1) < 1) {
    ...
    crs(ssp10) <- crs(tdaye)
    if (model == 'BCC-CSM2-MR') {
      time(ssp10) <- seq(as.Date('2015-01-16'), as.Date('2100-12-16'), by = 'month')
    }
    if (xmax(ssp10) > 200) {
      ssp_t20 <- rotate(ssp10)
    } else {
      ssp_t20 <- ssp10
    }
    
    ssp_t20 <- c(pure_hist_t2_2, ssp_t20) #
    ind <- which(time(ssp_t20) %in% seq(as.Date('2001-01-01'), as.Date('2300-12-31'), by =
                                          'day'))
    ssp_t30 <- ssp_t20[[ind]]
    ssp_t40 <- tapp(tapp(ssp_t30, index = rep(1:(
      length(ind) / (12)
    ), rep(
      12, length(ind) / (12)
    )), fun = sum),
    ...
    diff_ssp_1km_2 <- resample(bc_ssp1_2, tdaye, method = 'bilinear', threads =
                                 TRUE)
    f_ssp_1km_2  <- diff_ssp_1km_2 * ca_covas[[51]]
  }
  if (!is.null(diff_ssp0)) {
    lapply(1:nlyr(f_ssp_1km_2), function(x)
      writeRaster(
        f_ssp_1km_2[[x]],
        filename = paste0(
          '/mnt/DataSpace/Data_pool/05_smap_10/',
