#from and edit from  /mnt/File0/DAAATAAA/Data_collection/Canada_soil_archive/Canada_SOC_rebuild/003_code_lehigh/09_01_turnover_k_PLANT_PET_climate_sm_6.R
#
#
#
############################################################################
...
for (i in models) {
  for (scenario in c('ssp245', 'ssp585')) {
    ...
    for (k in 1:length(f_pet)) {
      ...
      #after add bed rock depth
      # model000 <- grep(pattern = glob2rx("*GBM*20240927*"), x = list.files(path = paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/traind_model/',mk),full.names = T),value=T) #lu he cardamom 排行第二2 是ensemble 因此就选这个吧
      # model000 <- grep(pattern = glob2rx("*StackedEnsemble*20241231*"),
      #                 x = list.files(path = paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/traind_model/',mk),full.names = T),value=T) #lu he cardamom 排行第二2 是ensemble 因此就选这个吧
      model000 <- grep(
        pattern = glob2rx("*StackedEnsemble*20250109*"),
        x = list.files(
          path = paste0(
            '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/traind_model/',
            mk
          ),
          full.names = T
        ),
        value = T
      ) #lu he cardamom 排行第二2 是ensemble 因此就选这个吧
      ...
      terra::writeRaster(
        maps2,
        paste0(
          '/mnt/Fastrun/Canada_C/k/all_2100_20250109/',
          mk,
          '_',
          'PLANT_PET_CLIMATE_SM_',
          i,
          '_',
          scenario,
          '_',
