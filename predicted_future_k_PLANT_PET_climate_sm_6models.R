...
#
##------------------------------------
#20240329 update some related to plant 
  ...
  for(scenario in c('ssp245','ssp585')) {
    ...
    for (k in 1:length(f_pet)) {  # as pet always 2100
     ...
     # model000 <- grep(pattern = glob2rx("*StackedEnsemble*20241231*"),
      ...
      model000 <- grep(pattern = glob2rx("*StackedEnsemble*20250109*"),
                       x = list.files(path = paste0('/mnt/DataSpace/Projects/Canada_C/Canada_C_final/traind_model/',mk),full.names = T),value=T) #lu he cardamom 排行第二2 是ensemble 因此就选这个吧
      ...
      GBM <- h2o.loadModel(model000)
        ...
        terra::writeRaster(maps2,
                           paste0('/mnt/Fastrun/Canada_C/k/all_2100_20250109/',mk,'_','PLANT_PET_CLIMATE_SM_',i,'_',scenario,'_',time00[k],'.tif'),overwrite = TRUE)
      # }
      h2o.shutdown(prompt = FALSE)
    }
  }}
