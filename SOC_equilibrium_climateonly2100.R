...
f_k <- list.files(
  '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/Climateonly_2100',
  full.names = T
)
...
(
  f_veglit <- list.files(
    pattern = 'tif',
    pat = '/mnt/Fastrun/Canada_C/litter_climate_only2100/',
    full.names = T
  )
)
    ...
    for (scenario in c('ssp245', 'ssp585')) {
      ...
      writeRaster(
        ...
        filename = paste0(
          ...
          '_',
          expt,
          '_based_on_sasa_two_box_balance_2060_2100status.tif'
        ),
        overwrite = TRUE
      )
      saveRDS(
        ...
        paste0(
          '/mnt/DataSpace/Projects/Canada_C/Canada_C_final/model_outputs/',
          name0,
          '_SOC_dynamic_fluxcom_gpp_climateonly2100_annual20240826_newlitter.rds'
        )
      )
    }
  }
