...
length(unique(use_layer3$profile_id)) #77417
#delete 0 value
...
# after es_spline
t1 <- dplyr::distinct(use_layer3_1[, c(1:6)]) #77417
############---------------------------------------------
num1 <- list()
...
use_data1 <- dplyr::left_join(use_data0[[1]], distinct_data0, by = c('id' =
                                                                       'profile_id')) # 之所以选1，3,4,5，6(即t2) 因为1和2不完全match 77411
saveRDS(
  use_data1,
  paste0(
    path00,
...
) # fullfil 165 layers and used later on, we rarely use use_data1
...
system.time(
  extractdata0 <- terra::extract(covs_list, use_data7)  # 338.444s)
  ...
  setwd(
    '/mnt/File0/DAAATAAA/Data_collection/MY_global_Temp_resilence_C'
  )
  data_use <- read.csv(
    './01Output/07base_for_model_all_20241212_UPDATE10CMinterval.csv'
  )
  ...
  set.seed(7180)
  test4 <- sample(1:100, 5) # update from 3 to 5
  ...
  ##step 5
  ca_covas <- rast(
    '/mnt/File0/DAAATAAA/forfinalmap_1km/canada_all_covs_global1214.tif'
  )
  gpp_hist <- rast(
  ...
  names(ca_peat_use1)[1:19] <- c('X5cm', 'X10cm', 'X15cm', 'X20cm', 'X30cm', paste0('X', c(
    seq(40, 100, 10), seq(120, 300, 30)
  ), 'cm'))
  ...
  d7 <- rbind(d6, t_test)
  names(d7)[1:19] <- c('X5cm', 'X10cm', 'X15cm', 'X20cm', 'X30cm', paste0('X', c(
    seq(40, 100, 10), seq(120, 300, 30)
  ), 'cm')) # 之前命名有问题
  d6 <- d7
  plot(world %>% filter(name_long == 'Canada') %>% st_geometry())
  ...
  d6$GPP4 <- gpp_new$lyr.1 # unite is yearly
  d1 <- d6
  d1$X.1 = 1:nrow(d1)
  ...
  d100 <- readRDS(paste0(
    path, 'ca_soc_traindataset_20241212.rds'
  ))
  d6 <- d100[, 1:78]
  ...
  peat_ca <- rast(
    '/mnt/DataSpace/Projects/Canada_C/processed_output/peatland_area1214.tif'
  ) #change name from peatland_area to peatland1214
  ...
  # 5   30   60  100
  #2892 2614  503  425
  reclass_table <- matrix(
    c(0, 5, 5, 5, 30, 30, 30, 60, 60, 60, 100, 100),
    ncol = 3,
    byrow = TRUE
  )
  ...
  d10 <-
    d1 %>% dplyr::select (all_of(item0), all_of(covs0), all_of(added_cov)) %>% #X ID :delete as no use
    tidyr::pivot_longer(
      cols = paste0("X", c(
        5, 10, 15, 20, 30, seq(40, 100, 10), seq(120, 300, 30)
      ), "cm"),
  ...
  coord00 <- rast(
    '/mnt/File0/DAAATAAA/forfinalmap_1km/canada_coordinates.tif'
  )
  sedimentary_thick <- rast(
    '/mnt/File0/DAAATAAA/forfinalmap_1km/Canada_sedimentary_thickness.tif'
  )
  ...
  reclass_table <- matrix(
    c(0, 5, 5, 5, 25, 25, 25, 50, 50, 50, 60, 60, 60, 100, 100),
    ncol = 3,
    byrow = TRUE
  )
  ...
  t2 <- rast(t1)
  plot(t2[[seq(1, 42, 7)]], breaks = c(
    0, 5, 10, 20, 30, 45, 60, 80, 100, 200, 300, 400, 600
  ))
  plot(t2[[seq(1, 42, 7)]])
  library(RColorBrewer)
  #display.brewer.all()
  col0 <- colorRampPalette(rev(brewer.pal(
    n = 8, name = 'Spectral'
  )))
  ...
  use_data4 <- use_data3 %>% group_by(cell) %>% summarise_at(
    vars('0-5 cm':'270-300 cm', 'latitude', 'longitude', 'x', 'y'),
    mean,
    na.rm = T
  )
  ...
  peat_reclassified <- classify(
    peat_ca,
    reclass_table,
    include.lowest = T,
    right = T
  )
  peat_reclassified <- terra::focal(
    peat_reclassified,
    w = 7,
    fun = 'modal',
    na.policy = 'only'
  ...
  reclass_table <- matrix(
    c(0, 5, 5, 5, 30, 30, 30, 45, 45, 45, 100, 100),
    ncol = 3,
    byrow = TRUE
  )
  
  
  peat_reclassified22 <- classify(
    peat_ca,
    reclass_table,
    include.lowest = T,
    right = T
  ...
  ### 20250115
  depth00 <- rast(
    '/mnt/File0/DAAATAAA/forfinalmap_1km/ca_bed_depth_of_each_of5layer.tif'
  )
  socs_ml <- f4 * BD0 * depth00
  socs_ml_1m <- sum(socs_ml[[1:5]]) / 100
  plot(socs_ml_1m)#
  ...
  ca_f1 <- sum(ca_f[[1:5]])
  ca_pure_area84 <- rast(
    '/mnt/DataSpace/Projects/Canada_C/processed_output/ca_cmip6_area_WGS84.tif'
  ) #new created
  ca_f2 <- ca_f1 * ca_pure_area84
  global(ca_f2, function(x)
    sum(x, na.rm = T)) / 10^14
  plot(ca_f1 / 100, breaks = c(
    0, 3, 5, 10, 20, 30, 40, 50, 70, 100, 150, 200, 400
  ))
  writeRaster(
    ca_f1 / 100,
